...
def align_midi_to_frames(midi_events, time_frames):
    ...
    for note in midi_events:
        ...
        if 0 <= pitch < 88:
            ...
            onset_frame = np.searchsorted(frame_intervals, start_time) - 1
            onset_frame = max(0, min(onset_frame, num_frames-1))

            offset_frame = np.searchsorted(frame_intervals, end_time) - 1
...
def segment_and_save_audio_midi(audio_path, midi_path, output_dir,
    ...
    for seg in range(num_segments):
        ...
        for label_type, label_data in labels.items():
            frames_mask = (time_frames >= segment_start_time) & (time_frames < segment_end_time)
            segment_frames = time_frames[frames_mask]

            segment_frames = segment_frames - segment_start_time

            segment_label_data = label_data[:, frames_mask]

            adjusted_labels[label_type] = segment_label_data
...
if __name__ == '__main__':
    ...
    spectrograms_directory = "../dataset/spectrograms_mel_20s_V2"
