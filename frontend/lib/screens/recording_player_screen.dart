...
class _RecordingPlayerScreenState extends State<RecordingPlayerScreen>
  ...
  Future<void> _saveChanges() async {
    ...
    try {
      ...
      // Use the new metadata update method
      ...
      // Update the widget data with new values
  ...
  Future<void> _generateTranscription({bool forceRegenerate = false}) async {
    ...
    try {
      ...
      // Get recording ID - try multiple possible locations
      final recordingId = widget.recordingData['recording_id'] ?? 
                         widget.recordingData['metadata']?['recording_id'] ??
                         widget.recordingData['id'];
      ...
      // Check if files already exist and not forcing regeneration
      if (!forceRegenerate) {
      ...
      // Call the transcription API for existing recording
      ...
      if (result != null) {
        ...
        // Refresh the recording data by refetching from server
  ...
  Future<void> _refreshRecordingDataSimple(String userId, String recordingId) async {
    try {
      ...
      // Find our recording in the updated list
      ...
      if (updatedRecording.isNotEmpty) {
        // Update the widget data with new file information
        widget.recordingData.clear();
        widget.recordingData.addAll(updatedRecording);
        
        // Trigger a rebuild to show new files
        setState(() {});
  ...
  // NEW: Refresh recording data after transcription
  ...
  // NEW: Download file functionality
  Future<void> _downloadFile(String url, String fileName) async {
    try {
      _showInfoMessage('Downloading $fileName...');
      
      // Extract clean filename from the label
  ...
  // NEW: Play MIDI file directly in the recording screen
  ...
  // NEW: Show transcription visualization by regenerating (avoids model constructor issues)
  Future<void> _showTranscriptionVisualization() async {
    try {
      ...
      _showInfoMessage('Loading transcription visualization...');

      // Simple approach: call the transcription API - it will return existing files if they exist
      final recordingId = widget.recordingData['recording_id'] ?? 
                         widget.recordingData['metadata']?['recording_id'] ??
                         widget.recordingData['id'];
      ...
      if (recordingId == null || userId == null) {
        _showErrorMessage('Recording information incomplete');
        return;
      }

      // Call the transcription API - it will return existing files without regenerating
      final result = await _apiService.transcribeExistingRecording(
        userId: userId,
        recordingId: recordingId,
        title: _titleController.text.trim(),
      ...
      if (result != null) {
        // Navigate to results screen with fresh data
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => ResultScreen(result: result),
          ),
        ...
        _showErrorMessage('Could not load transcription data');
      }

    } catch (e) {
      _showErrorMessage('Error loading transcription: $e');
      print('❌ Transcription visualization error: $e');
    }
  }
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // Show available files or empty state
          ...
          children: [
            ...
            Row(
              children: [
                ...
                // Regenerate button (if transcription exists)
                if (hasTranscription) ...[
                  SizedBox(width: 12),
                  Expanded(
                    flex: 1,
  ...
  // NEW: Show dialog to confirm regeneration
...
// import '../config/app_theme.dart';
// import 'piano_visualization_screen.dart';
...
//       // Încarcă audio din URL-ul de la AWS S3
//       await _audioPlayer.setUrl(widget.recordingUrl);

//       // Setează handler pentru stare
...
//       // Setează handler pentru durata totală
...
//       // Setează handler pentru poziția curentă
...
//       setState(() {
//         _isLoading = false;
//         _errorMessage = 'Eroare la încărcarea înregistrării: $e';
//       });
//     }
//   }
...
//   Widget build(BuildContext context) {
//     final metadata = widget.recordingData['metadata'] ?? {};
//     final title = metadata['title'] ?? 'Înregistrare fără titlu';
//     final isAnalyzed = metadata['analyzed'] == 'true' || metadata['analyzed'] == true;

//     // Obține data încărcării din metadate (dacă există)
//     String uploadDate = '';
//     if (metadata['uploadDate'] != null) {
//       try {
//         final date = DateTime.parse(metadata['uploadDate']);
...
//               // Buton pentru analiza notelor
//               if (!isAnalyzed)
//                 Padding(
//                   padding: const EdgeInsets.only(bottom: 16),
//                   child: ElevatedButton.icon(
//                     onPressed: () {
//                       // Aici va fi adăugată logica pentru analiza notelor
//                       ScaffoldMessenger.of(context).showSnackBar(
//                         SnackBar(
//                           content: Text('Sending for analysis...'),
//                           backgroundColor: AppTheme.primaryColor,
//                         ),
//                       );

//                       // Aici ar trebui să apelezi backend-ul pentru a declanșa analiza
//                       // De exemplu: _awsService.requestAnalysis(widget.recordingUrl, widget.recordingData['key']);
//                     },
//                     icon: Icon(Icons.auto_fix_high),
//                     label: Text('Analyse the notes'),
//                     style: ElevatedButton.styleFrom(
//                       backgroundColor: AppTheme.primaryColor,
//                       padding: EdgeInsets.symmetric(vertical: 16),
//                     ),
...
//               // Buton pentru vizualizarea Synthesia
//               ElevatedButton.icon(
//                 onPressed: () {
//                   Navigator.of(context).push(
//                     MaterialPageRoute(
//                       builder: (context) => PianoVisualizationScreen(
...
//                 label: Text('Visualizing Notes (Synthesia Style)'),
//                 style: ElevatedButton.styleFrom(
//                   backgroundColor: AppTheme.accentColor,
//                   padding: EdgeInsets.symmetric(vertical: 16),
//                 ),
